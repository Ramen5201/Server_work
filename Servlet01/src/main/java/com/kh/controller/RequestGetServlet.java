...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/**
		 * 첫 번째 매개변수인 request에는 요청시 전돨된 내용들 담겨있음(사용자가 입력한 값, 요청전송방식, 요청자의 ip주소등등)
		 * 두 번째 매개변수인 response 요청 처리 후 응답할 때 사용되는 객체
		 * 
		 * 요청 처리를 위해서 요청시 전달된 값들 추출
		 * request의 parameter영역 안에 존재 (키=벨류 세트로 담겨있음)
		 * 
		 * 따라서 request의 parameter영역으로부터 전달 된 데이터 추출하는 메소드
		...
		double height = Double.parseDouble(request.getParameter("height")); //"170" => 170.00~ 

		// 체크박스와 같이 복수개의 벨류값들을 추출하고자 할대
		...
		// 추출한 값(요청시 전달된 값)들을 가지고 요청처리를 해야됌(db와 상호작용)
		// > Service메소드 호출 > Dao메소드 > DB sql 실행
		
		/**
		 * 
		 * int result = new MemberSerivce().insertMember(name, gender, age, city, height, foods);
		 * 
		 * if(result > ){
		 *  // 성공 -> 성공페이지응답
		 *  } else {
		...
		// 장점 : Java코드 내에 작성하기 떄문에 반복문이나 조건문, 유용한 메소드 같은 걸 할용할 수 있다.
		// 단점 : 불편, 복잡, 혹시라도 차후에 html을 수정한다면 Java코드를 수정하는 것이기 떄문에 
		// 		 수정된 내용을 반영시키고자 서버를 재실행(restart) 해야됨
		
		
		// * response객체를 통해 사용에게 html(응답화면) 전달
